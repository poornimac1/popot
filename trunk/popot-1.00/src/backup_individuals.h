...
namespace popot
{
  ...
  namespace PSO
  {
	...
	virtual void print(std::ostream & os) const
	{
       ...
       */
      template< typename PROBLEM, typename PARTICLE_PARAMS, typename POSITION_INITIALIZER, typename VELOCITY_INITIALIZER>
	class TraditionalParticle : public Particle<PROBLEM, POSITION_INITIALIZER, VELOCITY_INITIALIZER>
      {
	typedef Particle<PROBLEM, POSITION_INITIALIZER, VELOCITY_INITIALIZER> TSuper;

      public:

	TraditionalParticle(void) : TSuper()
	{
	}
	 ...
	 * Destructor
	 */
	virtual ~TraditionalParticle(void)
	{
	}
	...
	virtual void updateVelocity(void)
	{
	  ...
	  // v = w * v + c1 r1 (best_p - p) + c2 r2 (best_g - p)
	  // with :
	  ...
	  for(int i = 0 ; i < TSuper::_dimension ; ++i)
	    {
	      ...
	      this->setVelocity(i, PARTICLE_PARAMS::w() * this->getVelocity(i)
				+ r1 * PARTICLE_PARAMS::c1() * (this->getBestPosition()->getPosition(i) - this->getPosition(i))
				+ r2 * PARTICLE_PARAMS::c2() * (this->getNeighborhood()->getBest()->getPosition(i) - this->getPosition(i)));
	    }
	}
      };
	...
	{
	  gi = new double[TSuper::_dimension];
	...
	virtual void updateVelocity(void)
	{
	  ...
	  // where x_i is sampled uniformely from the hypersphere (Gi, Ri)
	  // with Gi the center and Ri the radius and
	  // Gi = 1/3 ( x_i + (x_i + c(p_i - x_i)) + x_i + c(l_i - x_i))
	  //    = x_i + c (p_i + l_i - 2x_i)/3
	  // Ri = ||G_i - x_i||

	  // We first check if the local best and personal best are identical by computing the norm of the difference
	  // between both positions
	  double diff_li_pi = 0.0;
	  bool li_equals_pi = false;
	  for(int i = 0 ; i < TSuper::_dimension ; ++i)
	    diff_li_pi += (this->getBestPosition()->getPosition(i) - this->getNeighborhood()->getBest()->getPosition(i))
		*(this->getBestPosition()->getPosition(i) - this->getNeighborhood()->getBest()->getPosition(i));
	  li_equals_pi = (sqrt(diff_li_pi) <= 1e-5);

	  // We first compute the center of gravity
	  if(li_equals_pi)
	    {
	      for(int i = 0 ; i < TSuper::_dimension ; ++i)
		gi[i] = this->getPosition(i) + PARTICLE_PARAMS::c() * (this->getBestPosition()->getPosition(i)  - this->getPosition(i)) / 2.0;
	    }
	  else
	    {
	      for(int i = 0 ; i < TSuper::_dimension ; ++i)
		gi[i] = this->getPosition(i) + PARTICLE_PARAMS::c() * (this->getBestPosition()->getPosition(i) + this->getNeighborhood()->getBest()->getPosition(i) - 2.0 * this->getPosition(i)) / 3.0;
	  ...
	  ri = 0.0;
	  for(int i = 0 ; i < TSuper::_dimension ; ++i)
	    ri += (gi[i] - this->getPosition(i))*(gi[i] - this->getPosition(i));
	  ri = sqrt(ri);

	  // Compute the auxiliary position x'_i
	  // To uniformely sample from the hypersphere we uniformely sample a direction
	  double norm = 0.0;
	  for(int i = 0 ; i < TSuper::_dimension ; ++i)
	    {
	      xpi[i] = popot::math::uniform_random(0.0,1.0);
	      norm += xpi[i] * xpi[i];
	    }
	  
	  // And then scale by a random radius
	  double radius = popot::math::uniform_random(0.0,ri);
	  for(int i = 0 ; i < TSuper::_dimension ; ++i)
	    xpi[i] = gi[i] + xpi[i] * radius / norm;
      ...
      /*  *\/ */
      /* template< typename PROBLEM, typename PARAMS> */
      /* class FullyInformedParticle : public Particle<PROBLEM> */
      /* { */
      /* 	typedef Particle<PROBLEM> TSuper; */
      ...
      /* 	  popot::PSO::neighborhood::Neighborhood< Particle<PROBLEM> > * myneigh = this->getNeighborhood(); */
      /* 	  double r = 0.0; */
      /* 	  double r_sum = 0.0; */
      /* 	  double dv = 0.0; */
      ...
      /* 		  dv += r * (((Particle<PROBLEM>*)(myneigh->get(j)))->getBestPosition()->getPosition(i) - this->getPosition(i)); */
      /* 		} */
