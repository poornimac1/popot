...
//
	...
	// Display the best
//	printf( "\nBest value after init. %1.20e ", errorPrev );
//		printf( "\n Position :\n" );
...
*/
		...
		{	
			...
			{
				s1:			s1=alea_integer(0,param.S-1); if(s1==s) goto s1;	
			}
			...
			for (d = 0; d < pb.SS.D; d++) 
			{
				...
				switch(param.BW[1])
				{
					...
					case 1: //G is a random informant (see above). 
						...
						if(alea(0,1) <(double)param.S/(pb.SS.D*param.K))	// Approximation
						{ // Use P twice
