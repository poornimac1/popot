...
#define POS_MIN_PLATEAU -7.5
#define POS_MAX_PLATEAU 7.5
...
public:
  ...
  {
    return 2.0 * POS_MIN_PLATEAU;
  }

  static double get_ubound(int index)
  {
    return 2.0 * POS_MAX_PLATEAU;
  }

  static double evaluate(double * params)
  {
    if(params[0] >= POS_MIN_PLATEAU && params[0] <= POS_MAX_PLATEAU
       && params[1] >= POS_MIN_PLATEAU && params[1] <= POS_MAX_PLATEAU)
      return 800.0;
    else
      {

	double * new_params = new double[2];
	new_params[0] = params[0] - 10.0;
	new_params[1] = params[1] - 10.0;
	return popot::problems::Rastrigin<2>::evaluate(new_params);
      }
  }
};
...
{
  std::ofstream outfile("PlotExample005/function_values.data");
...
void save_particle_positions(int epoch, PSO & p)
{
  ...
  filename_plot << "PlotExample005/" << root_filename.str()
		<< ".plot";
...
int main(int argc, char* argv[]) {
  ...
  save_particle_positions(0, pso);
  for(int i = 1 ; i < 1000 & pso.getBest()->getFitness() >= 1e-7; ++i)
    {
      pso.step();
  ...
  std::cout << " To get an animated gif of the behavior of the PSO, and if you used the source package, go into PlotExample005 and type make all . It requires convert and gnuplot" << std::endl;
